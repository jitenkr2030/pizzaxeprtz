...
const sampleCartItems: CartItem[] = [
  {
    ...
    isSpicy: false,
    customizations: [
      { name: "Extra Cheese", price: convertUSDToINR(2.00) },
      { name: "Large Size", price: convertUSDToINR(3.00) }
    ]
  },
  {
    id: "2",
    ...
    isSpicy: false,
    customizations: [
      { name: "Extra Pepperoni", price: convertUSDToINR(3.00) }
    ]
  },
  {
    id: "5",
...
export default function CartPage() {
  ...
  const deliveryFee = subtotal > convertUSDToINR(20) ? 0 : convertUSDToINR(2.99)
  const tax = subtotal * 0.18 // GST rate in India
                              ...
                              <div className="mt-2">
                                ...
                                {item.customizations.map((custom, index) => (
                                  <p key={index} className="text-xs text-gray-600">
                                    + {custom.name} ({formatINRWithRs(custom.price)})
                                  </p>
                                ))}
                              </div>
                            )}
                        ...
                        <div className="flex items-center justify-between mt-4">
                          ...
                          <div className="text-right">
                            <p className="font-semibold text-lg text-red-600">
                              {formatINRWithRs((item.price + (item.customizations?.reduce((sum, custom) => sum + custom.price, 0) || 0)) * item.quantity)}
                            </p>
                            <p className="text-sm text-gray-500">
                              {formatINRWithRs(item.price)} each
                            </p>
                          </div>
                        </div>
                      </div>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span>{formatINRWithRs(subtotal)}</span>
                  </div>
                  
                  <div className="flex justify-between text-sm">
                    <span>Delivery Fee</span>
                    <span className={deliveryFee === 0 ? "text-green-600" : ""}>
                      {deliveryFee === 0 ? "FREE" : formatINRWithRs(deliveryFee)}
                    </span>
                  </div>
                  ...
                  {deliveryFee === 0 && (
                    <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                      <p className="text-sm text-green-800">
                        🎉 Free delivery on orders over {formatINRWithRs(convertUSDToINR(20))}!
                      </p>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between font-semibold text-lg">
                    <span>Total</span>
                    <span className="text-red-600">{formatINRWithRs(total)}</span>
                  </div>
                </div>
